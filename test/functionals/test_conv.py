...
def _test_convNd(N: int, case: Dict, device: device):
...
def test_conv1d(case: Dict, device: device):
    ...
    """
    N = 1
    _test_convNd(N, case, device)


@mark.parametrize("device", DEVICES, ids=DEVICE_IDS)
@mark.parametrize("case", CONV_2D_CASES, ids=CONV_2D_IDS)
def test_conv2d(case: Dict, device: device):
...
def test_conv3d(case: Dict, device: device):
...
def test_conv4d(case: Dict, device: device):
